...
func ParseTokenWithCustomer(token string) (*model.Customer, error) {
	claim, err := ParseToken(token)
	if err != nil {
		return nil, err
	}
	...
	repo.DB.First(c, claim.Audience)

	return c, nil
}

func GenJwtToken(c *model.Customer) (string, error) {
	expiresTime := carbon.Now().AddDays(7).Unix()
	tokenId := sn.UuidString()

	claims := &jwt.StandardClaims{
		Audience:  util.Interface2String(c.ID),    // 受众
